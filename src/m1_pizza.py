...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    t = 'DRAW_POINTS_ON_CIRCLE, test 4:  6 Black dots.'
    window = rg.RoseWindow(600, 600, t)
    circle = rg.Circle(rg.Point(169, 246), 172)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    p = p + generate_points_on_circle(circle, number_of_points)
    for k in range(len(p)):
        point = p[k]
        circle_new = rg.Circle(point, 10)
...
def run_test_pizza():
    ...
    # Test 4:
    t = 'PIZZA test 4:  1000 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, t)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'yellow'
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    s = generate_points_on_circle(circle, number_of_slices)
    for k in range(len(s)):
        begining = circle.center
        end = s[k]
        l = rg.Line(begining, end)
        l.color = color
...
def run_test_polygon():
    ...
    # Test 3
    window = rg.RoseWindow(600, 600, 'Test 3')
    circle = rg.Circle(rg.Point(321, 123), 209)
    circle.outline_thickness = 5
    polygon(window, circle, 7, 'purple', 3)
    window.close_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    t = 'FANCY POLYGON test 4:  62 lime green lines on blue circle, hops = 22.'
    window = rg.RoseWindow(600, 600, t)
    circle = rg.Circle(rg.Point(312, 301), 129)
    circle.fill_color = 'yellow'
    fancy_polygon(window, circle, 62, 22, 'green', 7)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(number_of_lines):
        s = p[k]
        e = p[(k + hops_to_next_point) % len(p)]
        l = rg.Line(s, e)
        l.color = color
        l.thickness = thickness
        l.attach_to(window)
